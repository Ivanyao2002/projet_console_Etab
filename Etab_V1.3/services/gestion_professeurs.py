...
class ServicesProfesseur:
    ...
    def listerProfesseurs():
        ...
        if professeurs:
            ...
            for prof in professeurs:
                id_prof = prof[0]
                date_naissance = prof[1].strftime('%d-%m-%Y')
                ville = prof[2]
                prenom = prof[3]
                nom = prof[4]
                telephone = prof[5]
                vacant = "Oui" if prof[6] == 1 else "Non" 
                matiereEnseigne = prof[7]
                prochainCours = prof[8]
                sujetProchaineReunion = prof[8]
                print(f"ID: {id_prof}, Nom: {nom}, Prénom: {prenom}, Date de naissance: {date_naissance}, Ville: {ville}, Téléphone: {telephone}, Vacant: {vacant}, Matière Enseignée: {matiereEnseigne}, Prochain Cours: {prochainCours}, Sujet Prochaine Réunion: {sujetProchaineReunion}")
    ...
    def modifierProfesseur():
        ...
        while True:
            try:
                identifiant = int(input("\033[0;33mEntrez l'identifiant du professeur à modifier : \033[0m"))
                ...
                while True:
                    vacant_input = input("\033[0;33mEst-ce vacant ? Oui(1) / Non(0) (Appuyez sur entrée pour garder l'ancienne valeur) : \033[0m").strip().lower()
                    if vacant_input in ['oui', '1']:
                        vacant = "Oui"
                        break
                    elif vacant_input in ['non', '0']:
    ...
    def supprimerProfesseur():
        ...
        identifiant = int(input("\033[0;33mEntrez l'identifiant du professeur à supprimer : \033[0m"))
        Professeur.supprimer(identifiant)

    def gestionProfesseurs():
        Menu.accueil("GESTION DES PROFESSEURS")
        while True:
            Menu.afficherSousMenuProf()
            try:
            ...
            match choix:
                case 1:
                    ServicesProfesseur.ajouterProfesseur()
                case 2:
                    ServicesProfesseur.supprimerProfesseur()
                case 3:
                    ServicesProfesseur.modifierProfesseur()
                case 4:
                    ServicesProfesseur.listerProfesseurs()               
                case 5:
                    break             
                case 0:
                    return                   
